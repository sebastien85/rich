package com.github.textutils.codec;

import java.util.HashMap;
import java.util.Map;

/**
 * 快速html/js/xml 反编码实体类
 */
public class HtmlDecodeRef {
    ...
    private static final String[][] ISO8859_1_ARRAY = { { "&nbsp;", "160" }, { "&iexcl;", "161" }, { "&cent;", "162" },
            { "&pound;", "163" }, { "&curren;", "164" }, { "&yen;", "165" }, { "&brvbar;", "166" },
            { "&sect;", "167" }, { "&uml;", "168" }, { "&copy;", "169" }, { "&ordf;", "170" }, { "&laquo;", "171" },
    ...
    private static final String[][] HTML40_ARRAY    = { { "&fnof;", "402" }, { "&Alpha;", "913" }, { "&Beta;", "914" },
    ...
    private HtmlDecodeRef(){
    }

    public static final HtmlDecodeRef INSTANCE = new HtmlDecodeRef();

    static {
        INSTANCE.addRefs(APOS_ARRAY);
        INSTANCE.addRefs(BASIC_ARRAY);
        INSTANCE.addRefs(ISO8859_1_ARRAY);
        INSTANCE.addRefs(APOS_ARRAY_HTML);
        INSTANCE.addRefs(HTML40_ARRAY);
    }

    private Map<String, Character>    map      = new HashMap<String, Character>();

    public static HtmlDecodeRef getInstance() {
        return INSTANCE;
    }

    public void addRefs(String[][] refArray) {
        for (int i = 0; i < refArray.length; i++) {
            char c = (char) Integer.parseInt(refArray[i][1]);
            addRef(c, refArray[i][0]);
        }
    }

    public void addRef(char charValue, String entiryName) {
        map.put(entiryName, charValue);
    }

    public Character getChar(String entiryName) {
        return map.get(entiryName);
    }
